...
//    \"limitNumber\": \"6\"
    ...
    if (subType == SubTypeMemory) {
        str = [NSString stringWithFormat:@"订阅%@的价格为%@学习豆！",title,priceStr];
    }else{
        str = [NSString stringWithFormat:@"一次性订阅%@所有单词的价格为%@学习豆！",title,priceStr];
    }
    ...
    _label.text = [NSString stringWithFormat:@"邀请%@个好友，价格低至%@学习豆。",discountDic[@"maxNumber"],discountPrice];
